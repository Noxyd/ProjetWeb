	...
	if($_POST['nature']=='credit'){//le cas ou on insere un credit
		...
		if ($nbre == 0){// on a pas recu de resultats càd qu'on s'est trompé dans le nom alos:
			setcookie("erreur_nom_source",1,time()+3, '/');//on pose un cookie
			header('location: ../budget2.php');//on le renvoie a la page budget.php
		}

		//etant ici ca veut dire qu'on a rentré le bon nom de l'organisme
		//insertion de l'operation dans la table flux
		$result = pg_prepare($bdd, "query", 'INSERT INTO flux(debit, credit, libelle, datef, idfin,ideq)VALUES ( 0, $1, $2, $3, $4,0)');
		$result1 = pg_execute($bdd, "query",array ($_POST['montant'],htmlentities($_POST['libelle']),$date,$_POST['source']));
		...
		if ($nbre == 0){
			...
			$result = pg_prepare($bdd, "query", 'INSERT INTO flux(debit, credit, libelle, datef, idfin,ideq)VALUES ( $1, 0, $2, $3, 0,$4)');
			$result1 = pg_execute($bdd, "query",array ($_POST['montant'],htmlentities($_POST['libelle']),$date,$_POST['source']));
		}
	}
	pg_close($bdd);//fermeture de la conexion
	header('location: ../budget2.php');//on redirige vers la table flux
